...
<div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
    <div>
        <h1 class="h4 mb-1">Quote <?= e($quote['number']) ?></h1>
        <span class="badge text-bg-<?= \App\Models\Quote::STATUS_COLORS[$display] ?>"><?= e(\App\Models\Quote::STATUSES[$display]) ?></span>
    </div>
    <a href="<?= route('portal.quotes.pdf', ['id' => $quote['id']]) ?>" class="btn btn-outline-brand"><i class="bi bi-download"></i> Download PDF</a>
</div>
...
<?php if ($invoice): ?>
    <div class="card border-success mb-3">
        <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <div class="fw-bold"><i class="bi bi-check-circle text-success"></i> You accepted this quote<?= $quote['accepted_at'] ? ' on ' . e(substr((string) $quote['accepted_at'], 0, 10)) : '' ?>.</div>
            ...
            <a href="<?= route('portal.invoices.show', ['id' => $invoice['id']]) ?>" class="btn btn-brand"><i class="bi bi-receipt"></i> View Invoice <?= e($invoice['number']) ?></a>
        </div>
    </div>
<?php elseif ($quote['status'] === \App\Models\Quote::STATUS_DECLINED): ?>
    <div class="alert alert-secondary">
        <i class="bi bi-x-circle"></i> You declined this quote<?= $quote['declined_at'] ? ' on ' . e(substr((string) $quote['declined_at'], 0, 10)) : '' ?>. Get in touch any time if you'd like it revisited.
    </div>
...
<?php if ($acceptable): ?>
    <div class="card">
        ...
        <div class="card-body">
            <p class="text-muted">Accepting this quote raises the invoice so we can get started. You'll see the invoice here straight away.</p>
            <div class="d-flex flex-wrap gap-2">
                <form method="post" action="<?= route('portal.quotes.accept', ['id' => $quote['id']]) ?>" onsubmit="return confirm('Accept this quote and raise the invoice?')">
                    <?= csrf_field() ?>
    ...
    <div class="modal fade" id="declineModal" tabindex="-1">
        <div class="modal-dialog">
            <form method="post" action="<?= route('portal.quotes.decline', ['id' => $quote['id']]) ?>" class="modal-content">
                <?= csrf_field() ?>
