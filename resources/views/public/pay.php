...
<div class="row justify-content-center">
    <div class="col-lg-9">
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
            <div>
                <h1 class="h3 mb-1">Invoice <?= e($invoice['number']) ?></h1>
            ...
            <a href="<?= route('pay.pdf', ['token' => $invoice['public_token']]) ?>" class="btn btn-outline-brand"><i class="bi bi-download"></i> Download PDF</a>
        </div>
            ...
            <div class="card-body">
                <div class="row mb-3">
                    ...
                    <div class="col-sm-6 text-sm-end">
                        ...
                        <?php if (! empty($client['abn'])): ?><div class="small text-muted">ABN <?= e($client['abn']) ?></div><?php endif; ?>
                        <div class="small text-muted">Issued <?= e($invoice['issue_date']) ?> · Due <?= e($invoice['due_date']) ?></div>
                    </div>
                </div>
                ...
                <?php foreach ($instructions as $instruction): ?>
                    <div class="col-md-6">
                        <div class="card h-100">
                            <div class="card-body">
                                <h3 class="h6 mb-3"><?= e($instruction->label) ?></h3>
                                <?php if ($instruction->type === 'bank_transfer'): ?>
                                    ...
                                    <?php else: ?>
                                        <p class="text-muted small mb-0">Not configured yet.</p>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <?php if ($instruction->available): ?>
                                    ...
                                    <?php endif; ?>
                                <?php endif; ?>
                                <?php if ($instruction->available && $instruction->note): ?>
                                    <p class="text-muted small mb-0 mt-2"><?= e($instruction->note) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
